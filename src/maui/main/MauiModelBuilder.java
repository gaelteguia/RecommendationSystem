...
public class MauiModelBuilder implements OptionHandler {
	...
	/**
	 * Use all Wikipedia features Inverse Wikipedia frequency & Semantic relatedness
	 */
	...
	public Enumeration<Option> listOptions() {
		...
		newVector.addElement(
				new Option("\tSpecifies vocabulary format (text or skos or none).", "f", 1, "-f <vocabulary format>"));
		...
		try {
			...
			for (String file : dir.list()) {
				if (file.endsWith(".txt")) {
					...
					File keys = new File(inputDirectoryName + "/" + stem + ".key");
					if (keys.exists()) {
						stems.add(stem);
					}
				}
