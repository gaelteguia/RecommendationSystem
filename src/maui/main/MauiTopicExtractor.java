 ...
 * Specifies the minimum score threshold that phrases should accomplish to
 * appear in the output (default: 0).
 * <p>
 *
...
public class MauiTopicExtractor implements OptionHandler {
	 ...
	 *
	 * -n<br>
	 * Specifies maximum number of phrases to be output (default: 10).
	 * <p>
	 *
	 * -c "score from 0 to 1 (float)"<br>
	 * Specifies the minimum score threshold that phrases should accomplish to
	...
	public Enumeration<Option> listOptions() {
		...
		newVector.addElement(new Option("\tSpecifies document language (en (default), es, de, fr).", "i", 1,
				"-i <document language>"));
		newVector.addElement(new Option("\tSpecifies number of phrases to be output (default: 10).", "n", 1, "-n"));
		...
		newVector.addElement(new Option("\tSet the stopwords class to use (default: EnglishStopwords).", "s", 1,
				"-s <name of stopwords class>"));
		newVector.addElement(new Option("\tTurns debugging mode on.", "d", 0, "-d"));
		newVector.addElement(
				new Option("\tBuilds global dictionaries for computing TFIDF from the test collection.", "b", 0, "-b"));
		newVector.addElement(
				new Option("\tPrints graph description into a \".gv\" file, in GraphViz format.", "p", 0, "-p"));
	...
	public void extractKeyphrases(HashSet<String> fileNames) throws Exception {
		...
		for (String fileName : fileNames) {
				...
				if (topRankedInstances[i] != null) {
					...
					if (printer != null) {
						...
						if (additionalInfo) {
							...
							printer.print("\t");
							printer.print(Utils
									.doubleToString(topRankedInstances[i].value(mauiFilter.getProbabilityIndex()), 4));
						}
						printer.println();
					}
					if (debugMode) {
			...
			// correct/recall should be computed even if numExtracted==0, in such case would
			// be 0
			if (debugMode) {
		...
		if (correctStatistics.size() != 0) {
			...
			System.err.println("Precision: " + Utils.doubleToString(avgPrecision * 100, 2) + " +/- "
					+ Utils.doubleToString(stdDevPrecision * 100, 2) + " (skipped "
					+ (correctStatistics.size() - precisionStatistics.size()) + " document(s))");
	...
	/**
	 * Prints out a plain-text representation of a graph representing the main
	 * topics of the document. The nodes are the topics and the edges are relations
	 * between them as computed using the Wikipedia Miner. Only possible if
	 * Wikipedia data is provided.
	 * 
	 * @param topics
	 * @param root
	 * @param outputFile
						...
						try {
							...
							if (!done.contains(relation2) && !done.contains(relation)) {
								...
								if (relatedness < 0.2) {
									...
									printer.print(
											relation + "\" [penwidth = \"" + (int) (relatedness * 10 - 0.2) + "\"];\n");
								}
							}
